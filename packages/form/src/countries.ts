 ...
 *
 * Country names use English exonyms (the form most international
 * UI uses). Calling codes are stored without the leading `+` so
 * downstream code can prefix-match them against E.164 input
 * directly.
 */
export interface CountryInfo
{
    ...
    /** ITU-T E.164 calling code, without the leading `+`. */
    callingCode: string;
}
