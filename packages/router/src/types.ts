// ============================================================================
// AZEROTHJS — Router Type Definitions
// ============================================================================
//
// All shared types for the router live here. Component-specific
// prop types (LinkProps, RouteProps, OutletProps) live with their
// components; the Router object's shape lives next to createRouter.
//
// MENTAL MODEL:
//
//   The browser URL is the source of truth.
//   The history adapter wraps it so we can subscribe to changes.
//   The router parses the URL into a `RouteLocation` snapshot.
//   The location is exposed as a reactive signal.
//   Composables (useParams, useQuery) derive narrower memos.
//
//   URL change ──► HistoryAdapter ──► RouteLocation signal ──►
//                                                  ├── useRoute()
//                                                  ├── useParams()
//                                                  ├── useQuery()
//                                                  └── <Routes>/<Route>
 ...
 *
 * Values are always strings — the matcher does not coerce types.
 * Use `Number(params.id)` if you need a number.
 *
 * @example
 * ```ts
 ...
 * Parsed query string.
 *
 * Single values are strings. Repeated keys (`?tags=a&tags=b`)
 * produce a string array. Empty values are kept as empty strings,
 * not dropped — `?flag=` becomes `{ flag: '' }`.
 *
 * @example
 * ```ts
 * // ?page=2&sort=desc
 ...
 *
 * Returned by `useRoute()` and exposed as the value of the
 * router's `location` signal. Read-only — to navigate, use
 * `navigate()` from the router.
 *
 * Field names follow the browser `URL` API where possible
 * (`pathname`, `search`, `hash`) so the mental mapping is
 * obvious.
 */
 ...
 *
 * Layout components (routes that have `children`) receive
 * `children` containing the next nested level's rendered output;
 * leaf components ignore it. By default the router passes `{}`,
 * so a leaf component can declare `() => h('div', ...)` with no
 * argument and TypeScript stays happy.
 *
 * @example
 * ```ts
...
export interface Route
{
     ...
     *
     * Top-level routes are absolute (start with `/`); nested
     * routes are relative to their parent (no leading `/`). An
     * empty string `''` matches the parent path exactly (the
     * "index" route).
     *
     * Supported syntax:
    ...
    /** Optional nested routes — match against the unmatched suffix. */
    children?: Route[];
 ...
 *
 * `matched` is the chain from root to leaf — `<Outlet>` walks
 * this chain so each layout renders the next step.
 *
 * Internal-leaning, but exported so advanced users (route
 * guards, tooling) can inspect what matched.
 */
export interface RouteMatch
{
    ...
    /** Root → leaf chain of every route that matched along the way. */
    matched: Route[];
 ...
 *
 * A plain string is treated as a `fullPath` (pathname plus
 * optional search and hash). The structured form is convenient
 * when building URLs from data — the router handles encoding.
 *
 * @example
 * ```ts
 * navigate('/users/42');
...
{
    /**
     * If `true`, replace the current history entry instead of
     * pushing a new one. The Back button will skip over this
    ...
    /**
     * If `true`, scroll the page to top after navigation. The
     * router does NOT scroll automatically by default — set this
     * to opt in for individual navigations, or wire your own
     * scroll-restoration logic via the location signal.
     * Default: `false`.
     */
    scroll?: boolean;
}
 ...
 * Routing strategy.
 *
 * For v1 only `'history'` is implemented (HTML5 History API:
 * `pushState` + `popstate`). The union is open so adding
 * `'hash'` or `'memory'` later is a non-breaking change.
 */
...
/**
 * The history abstraction the router uses to read and update the
 * URL.
 *
 * Browsers get the History API implementation. Tests get an
 * in-memory implementation. SSR gets a request-bound
 * implementation. The router itself stays oblivious to which one
 * it's talking to.
 */
export interface HistoryAdapter
{
     ...
     *
     * Always reflects the current state — the adapter never
     * lags behind the underlying source.
     */
    current(): string;
     ...
     *
     * The callback receives the new full path. Returns an
